...
void modification_des_distances(MatriceDistances matrice, size_t colonne){
	...
	for(size_t ligne = 0; ligne < nbPoint; ligne++){//parcours des points
		if(ligne > colonne){
			verifier_element_dans_matrice(nbPoint,ligne,colonne);
			indice_matrice = obtenir_indice_matrice(ligne,colonne);//ligne,colonne
		}else if(ligne < colonne){
			verifier_element_dans_matrice(nbPoint,colonne,ligne);
			indice_matrice = obtenir_indice_matrice(colonne,ligne);//colonne,ligne
		}
...
void echangerPoint(MatriceDistances matrice, size_t ligne, size_t colonne){
	if(ligne!=colonne){
		//printf(" ");
		Point *pointA = obtenir_element_tableau_points(tableau_points_matrice(matrice), ligne);
		Point *pointB = obtenir_element_tableau_points(tableau_points_matrice(matrice), colonne);
		Point *echange = pointA;
		pointA = pointB;
		pointB = echange;
		
		modification_des_distances(matrice,ligne);
