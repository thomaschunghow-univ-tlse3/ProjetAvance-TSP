...
void tournee_traitement(FILE *sortie, Options options, MatriceDistance matrice)
{
    ...
    case GENETIQUE_GENERIQUE:
        tournee_gestionnaire(sortie, options.nom_fichier_entree, matrice, GENETIQUE_GENERIQUE, &tournee_genetique);
        break;
...
size_t tournee_nb_sommet(Resultat *resultat){
	return permutation_obtenir_taille(resultat -> permutation);
}
...
size_t tournee_sommet_numero(Resultat *resultat, size_t indice){
	return permutation_obtenir_sommet(resultat-> permutation,indice);
}
