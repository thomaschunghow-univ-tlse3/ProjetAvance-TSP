...
Resultat tournee_genetique_generique(MatriceDistance m,size_t nombre_individus,size_t nombre_generations,double taux_mutation){
    ...
    for (size_t i = 0; i < nombre_individus; i++){ // Initialisation de la population
    ...
    for (size_t i = 0; i < nombre_individus; i++){ // Évaluation initiale
    ...
    for (size_t g = 0; g < nombre_generations; g++){ // Boucle principale de l'algorithme génétique
        ...
        if (taille_tournoi > nombre_individus) taille_tournoi = nombre_individus;

        for (size_t i = 0; i < nombre_individus; i++){ // Création de la nouvelle génération
